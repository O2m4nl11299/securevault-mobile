...
class _DownloadScreenState extends State<DownloadScreen> {
  ...
  Future<void> _startDownload() async {
    ...
    if (parsed == null) {
      setState(() => _error = 'Geçersiz veya tanınmayan SecureVault linki.');
      return;
    }
    if (parsed.pwdHash != null) {
      setState(() => _error =
          'Bu link ek şifre korumalı — mobil uygulamada bu özellik henüz '
          'desteklenmiyor. Linki bir tarayıcıda açın.');
      return;
    }
    ...
    try {
      ...
      if (savedPath != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Kaydedildi: $savedPath')),
        );
      }
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Dosya İndir')),
      body: SafeArea(
    ...
    return Column(
      ...
      children: [
        ...
        if (busy) ...[
          ...
          Text(
            'İndiriliyor ve şifresi çözülüyor... %${(_progress * 100).toStringAsFixed(0)}',
          ...
          FilledButton(
            ...
            child: const Text('İndir ve Şifresini Çöz'),
          ),
        const SizedBox(height: 16),
        const Text(
          'Not: link tek kullanımlıktır — bir kez indirildikten sonra '
          'tekrar çalışmaz.',
          textAlign: TextAlign.center,
    ...
    return Column(
      ...
      children: [
        ...
        if (canDirectSave) ...[
          ...
          const SizedBox(height: 8),
          const Text(
            'Dosya büyük olduğu için doğrudan kaydetme yerine paylaşım '
            'menüsünü kullanın (örn. bir dosya yöneticisine veya Drive\'a).',
            textAlign: TextAlign.center,
            style: TextStyle(color: Colors.grey, fontSize: 12),
          ),
        ],
        ...
        OutlinedButton(
          onPressed: _reset,
          child: const Text('Başka bir link indir'),
        ),
      ],
    );
  }
